...
# cf-toolkit: version matches VERSION, help/errors, subcommand dispatch
```

CI (`.github/workflows/ci.yml`) runs the syntax check, ShellCheck, and the offline suite on
`macos-latest` for every push and PR — keep it green. See `docs/HOWTO.md` §6 for expected
behaviors. A live bogus-token `cf-guard: BLOCKED`
